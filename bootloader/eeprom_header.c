...
extern uint32_t __bootloader_plugin_size;
extern uint32_t __bootloader_plugin_offset;
...
// needs "used" attribute to be kept by compiler, and KEEP() in linker script
__attribute__((used, section(".romboot_header"))) static const struct la9310_boot_header romboot_header = {
    ...
    .plugin_size = (uint32_t)&__bootloader_plugin_size,
    .plugin_offset = (uint32_t)&__bootloader_plugin_offset,
    ...
    .flags = BOOTROM_EARLY_EXIT | BOOTROM_NO_EDMA_USE_MEMCPY // Boot options
};
