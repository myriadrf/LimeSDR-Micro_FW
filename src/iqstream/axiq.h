...
#define CCSR_BASE 0x40000000
#define VSPA_CCSR_BASE (CCSR_BASE + 0x1000000)
#define VSPA_GPIN0_BASE (VSPA_CCSR_BASE + 0x500)
#define VSPA_GPOUT0_BASE (VSPA_CCSR_BASE + 0x580)
...
#define GPI(x) ((uint32_t)(VSPA_GPIN0_BASE + (x)*4))
...
#define GPO(x) ((uint32_t)(VSPA_GPOUT0_BASE + (x)*4))

static inline uint32_t iord(uint32_t addr)
{
    return IN_32(addr);
}

static inline uint32_t iord_masked(uint32_t addr, uint32_t bitmask)
{
    return IN_32(addr) & bitmask;
}

static inline void iowr(uint32_t addr, uint32_t data)
{
    OUT_32(addr, data);
}

static inline void iowr_masked(uint32_t addr, uint32_t mask, uint32_t data)
{
    uint32_t temp = IN_32(addr);
    temp &= ~mask;
    temp |= data & mask;
    OUT_32(addr, temp);
}
...
//     return iord_masked(AXIQ_CR0, AXIQ_CR_FIELD_FIFO << axiq_cr_shift(fifo));
// }
...
//     iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_FIFO << axiq_cr_shift(fifo), ctrl << axiq_cr_shift(fifo));
// }
...
static inline void axiq_fifo_rx_cr(enum axiq_bank_e bank, enum axiq_fifo_e fifo, uint32_t mask, uint32_t ctrl) {
    iowr_masked(AXIQ_CR0, (AXIQ_CR_FIELD_FIFO & mask) << axiq_cr_shift(fifo), ctrl << axiq_cr_shift(fifo));
}
...
//     return iord_masked(AXIQ_SR0, AXIQ_SR_FIELD_FIFO << axiq_sr_shift(fifo));
// }
...
static inline uint32_t axiq_fifo_rx_sr(enum axiq_bank_e bank, enum axiq_fifo_e fifo, uint32_t mask) {
    return iord_masked(AXIQ_SR0, (AXIQ_SR_FIELD_FIFO & mask) << axiq_sr_shift(fifo));
}
...
//     return iord_masked(AXIQ_CR3, AXIQ_CR_FIELD_FIFO << axiq_cr_shift(fifo));
// }
...
//     iowr_masked(AXIQ_CR3, AXIQ_CR_FIELD_FIFO << axiq_cr_shift(fifo), ctrl << axiq_cr_shift(fifo));
// }
...
static inline void axiq_fifo_tx_cr(enum axiq_bank_e bank, enum axiq_fifo_e fifo, uint32_t mask, uint32_t ctrl) {
    iowr_masked(AXIQ_CR3, (AXIQ_CR_FIELD_FIFO & mask) << axiq_cr_shift(fifo), ctrl << axiq_cr_shift(fifo));
}
...
//     return iord_masked(AXIQ_SR1, AXIQ_SR_FIELD_FIFO << axiq_sr_shift(fifo));
// }
...
static inline uint32_t axiq_fifo_tx_sr(enum axiq_bank_e bank, enum axiq_fifo_e fifo, uint32_t mask) {
    return iord_masked(AXIQ_SR1, (AXIQ_SR_FIELD_FIFO & mask) << axiq_sr_shift(fifo));
}
...
static inline void axiq_rx_disable() {
    // Disable RX FIFOs:
    iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_ENABLE_RX_ALL, AXIQ_CR_DISABLE_RX_ALL);
}
...
static inline void axiq_tx_disable() {
    // Disable TX FIFOs:
    iowr_masked(AXIQ_CR3, (AXIQ_CR_FIELD_ENABLE << AXIQ_CR_SHIFT_TX0), (AXIQ_CR_DISABLE << AXIQ_CR_SHIFT_TX0));
}
...
static inline void axiq_rx_enable() {
    // ENABLE RX FIFOs:
    iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_ENABLE_RX_ALL, AXIQ_CR_ENABLE_RX_ALL);
}
...
static inline void axiq_tx_enable() {
    ...
    // ENABLE TX FIFOs:
    iowr_masked(AXIQ_CR3, (AXIQ_CR_FIELD_ENABLE << AXIQ_CR_SHIFT_TX0), (AXIQ_CR_ENABLE << AXIQ_CR_SHIFT_TX0));
}
...
static inline void axiq_rx_iq_swap_on() {
    // ENABLE RX FIFOs:
    iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_IQ_SWAP_RX_ALL, AXIQ_CR_IQ_SWAP_RX_ALL_ON);
}
...
static inline void axiq_rx_iq_swap_off() {
    // ENABLE RX FIFOs:
    iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_IQ_SWAP_RX_ALL, AXIQ_CR_IQ_SWAP_RX_ALL_OFF);
}
...
static inline struct axiq_bank_error_t axiq_bank_error(enum axiq_bank_e bank) {
    ...
    error.rx = iord_masked(AXIQ_SR0, AXIQ_SR_FIELD_ERRUNDER_RX_ALL | AXIQ_SR_FIELD_ERROVER_RX_ALL);
    error.tx = iord_masked(AXIQ_SR1, (AXIQ_SR_FIELD_ERRUNDER << AXIQ_SR_SHIFT_TX0) | (AXIQ_SR_FIELD_ERROVER << AXIQ_SR_SHIFT_TX0));
    return error;
}

static inline void axiq_bank_clrerr(enum axiq_bank_e bank) {
    // Clear RX FIFOs errors:
    iowr_masked(AXIQ_CR0, AXIQ_CR_FIELD_CLRERR_RX_ALL, AXIQ_CR_CLRERR_RX_ALL);
    // Clear TX FIFOs errors:
    iowr_masked(AXIQ_CR3, (AXIQ_CR_FIELD_CLRERR << AXIQ_CR_SHIFT_TX0), (AXIQ_CR_CLRERR << AXIQ_CR_SHIFT_TX0));
}
